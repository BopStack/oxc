...
describe('no_hardcoded_colors', () => {
	test('detects hex color #FFF', () => {
		const result = runRule(no_hardcoded_colors, 'Literal', str_literal('#FFF'));
	...
	test('detects hex color #FFFFFF', () => {
		const result = runRule(no_hardcoded_colors, 'Literal', str_literal('#FFFFFF'));
		expectDiagnostics(result, [
			{ message: 'Avoid hardcoded colors. Use a theme/design-token instead.' }
		]);
	});
	...
	test('detects lowercase hex #ff0000', () => {
		const result = runRule(no_hardcoded_colors, 'Literal', str_literal('#ff0000'));
		expectDiagnostics(result, [
			{ message: 'Avoid hardcoded colors. Use a theme/design-token instead.' }
		]);
	});
	...
	test('detects rgb() pattern', () => {
		const result = runRule(no_hardcoded_colors, 'Literal', str_literal('rgb(255, 0, 0)'));
		expectDiagnostics(result, [
			{ message: 'Avoid hardcoded colors. Use a theme/design-token instead.' }
		]);
	});
	...
	test('detects rgba() pattern', () => {
		const result = runRule(no_hardcoded_colors, 'Literal', str_literal('rgba(255, 0, 0, 0.5)'));
		expectDiagnostics(result, [
			{ message: 'Avoid hardcoded colors. Use a theme/design-token instead.' }
		]);
	});
	...
	test('detects hsl() pattern', () => {
		const result = runRule(no_hardcoded_colors, 'Literal', str_literal('hsl(0, 100%, 50%)'));
		expectDiagnostics(result, [
			{ message: 'Avoid hardcoded colors. Use a theme/design-token instead.' }
		]);
	});
	...
	test('detects hsla() pattern', () => {
		const result = runRule(no_hardcoded_colors, 'Literal', str_literal('hsla(0, 100%, 50%, 0.5)'));
		expectDiagnostics(result, [
			{ message: 'Avoid hardcoded colors. Use a theme/design-token instead.' }
		]);
	});
