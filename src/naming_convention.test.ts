...
function binding_prop(key: string, local: string, shorthand = false, computed = false): unknown {
	return {
		type: 'Property',
		kind: 'init',
		key: { type: 'Identifier', name: key },
...
describe('naming_convention', () => {
		...
		const result = runRule(
			...
			'VariableDeclaration',
			{
				...
				declarations: [
					{
						type: 'VariableDeclarator',
						id: object_pattern([binding_prop('sourceKey', 'localName', false, false)]),
						init: null
					}
				]
			}
	...
	test('fails camelCase catch param', () => {
		const result = runRule(naming_convention, 'CatchClause', catch_clause(binding_ident('error')));
		expectDiagnostics(result, [{ message: 'Use snake_case for value names.' }]);
	});
	...
	test('fails camelCase const binding', () => {
		const result = runRule(naming_convention, 'VariableDeclaration', var_decl('const', 'maxRetries'));
	...
	test('fails non-PascalCase interface', () => {
		const result = runRule(naming_convention, 'TSInterfaceDeclaration', interface_decl('myInterface'));
		expectDiagnostics(result, [{ message: 'Use PascalCase for type names.' }]);
	});
	...
	test('passes CONSTANT_CASE const binding', () => {
		const result = runRule(naming_convention, 'VariableDeclaration', var_decl('const', 'MAX_RETRIES'));
		expectNoDiagnostics(result);
	});
	...
	test('ignores import specifiers with camelCase', () => {
		const result = runRule(
			naming_convention,
			'ImportDeclaration',
			import_decl('some-lib', [
				import_specifier('camelCaseExport', 'camelCaseExport')
			])
		);
		expectNoDiagnostics(result);
	});
		...
		const result = runRule(
			...
			'VariableDeclaration',
			{
				...
				declarations: [
					{
						type: 'VariableDeclarator',
						id: object_pattern([binding_prop('firstName', 'first_name', false, false)]),
						init: null
					}
				]
			}
